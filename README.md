...
* [August 20, 2024] Sometimes, to answer a multiple-choice question, students need to execute code (e.g., to perform calculations, apply a statistical test) to answer the question. Instructors can configure an exercise so that it includes a "sandbox" on the page. With this option, students can execute code, but the code is not evaluated directly.
...
* [April 16, 2026] Added buttons on assignment pages to navigate to the next or previous assignment.
...
* [April 30, 2026] Students can request an accommodation for an assignment directly via CodeBuddy. They can either request that they not be penalized for submitting past the due date or that they have time-and-a-half on an assignment (typically, an exam or quiz). When students submit this form, it sends an email to the instructor, and the instructor only needs to click on a link to approve it.
* [April 30, 2026] Instructors can now make bulk edits to assignments or exercises. For example, they can change all of the exercise titles in an assignment or all of the assignment titles in a course on a single page. There are various other options for making bulk edits.
* [April 30, 2026] Instructors can indicate that students are able to download their submissions for an assignment. Previously, this was a course-level option. Now it is assignment specific, and the default is "No".
* [September 10, 2026] Added ability to secure an assignment with a QR code. When configured for this, students must scan it and log in to their account to unlock the assignment and thus be able to access it. Also simplified the security code functionality so that security codes are always different for every student and provide a confirmation code.
