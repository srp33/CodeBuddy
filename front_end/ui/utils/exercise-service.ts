...
export async function submitCode(courseID: number, assignmentID: number, exerciseID: number, userCode: string): Promise<SubmitCodeResponse> {
	let url = `/submit/${courseID}/${assignmentID}/${exerciseID}`;
    const data = { "code": userCode, "date": (new Date()).toLocaleString("en-US", {timeZone: "UTC"}), partner_id: ''};
