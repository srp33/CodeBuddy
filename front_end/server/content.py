...
'''
    ...
    def specify_course_details(self, course_details, introduction, passcode, email_address, highlighted, virtual_assistant_config, templates, date_created, date_updated):
    ...
    def get_course_details(self, course_id):
        null_course = {"introduction": "", "passcode": None, "date_created": None, "date_updated": None, "email_address": "", "highlighted": False, "virtual_assistant_config": None, "templates": {}}

        if not course_id:
            return null_course

        sql = '''SELECT introduction, passcode, date_created, date_updated, email_address, highlighted, virtual_assistant_config, templates
                 FROM courses
                 WHERE course_id = ?'''
        ...
        course_details = {"introduction": row["introduction"], "passcode": row["passcode"], "date_created": row["date_created"], "date_updated": row["date_updated"], "email_address": row["email_address"], "highlighted": row["highlighted"], "virtual_assistant_config": row["virtual_assistant_config"]}
    ...
    def get_exercise_details(self, course_basics, assignment_basics, exercise_id):
        ...
        sql = '''SELECT instructions, back_end, output_type, allow_any_response, solution_code, solution_description, hint, max_submissions, starter_code, credit, data_files, what_students_see_after_success, date_created, date_updated, enable_pair_programming, show_upload_button, verification_code, weight, min_solution_length, max_solution_length
                 FROM exercises
                 WHERE course_id = ?
                   AND assignment_id = ?
                   AND exercise_id = ?'''
    ...
    def save_course(self, course_basics, course_details):
        ...
        if course_basics["exists"]:
            sql = '''UPDATE courses
                     SET title = ?, visible = ?, introduction = ?, passcode = ?, email_address = ?, highlighted = ?, virtual_assistant_config = ?, templates = ?, date_updated = ?
                     WHERE course_id = ?'''
            ...
            course_basics["id"] = self.execute(sql, (course_basics["title"], course_basics["visible"], course_details["introduction"], course_details["passcode"], course_details["email_address"], course_details["highlighted"], course_details["virtual_assistant_config"], templates_value, course_details["date_created"], course_details["date_updated"], ))
    ...
    def save_exercise(self, exercise_basics, exercise_details):
        ...
        if "show_upload_button" not in exercise_details:
            exercise_details["show_upload_button"] = 0
            ...
            if exercise_basics["exists"]:
                sql = '''
                    ...
                    SET title = ?, visible = ?, solution_code = ?, solution_description = ?, hint = ?,
                        ...
                        date_updated = ?, enable_pair_programming = ?, show_upload_button = ?, verification_code = ?,
                        weight = ?,
                        min_solution_length = ?,
                        max_solution_length = ?
                    WHERE course_id = ?
                ...
                sql = '''INSERT INTO exercises (course_id, assignment_id, title, visible, solution_code, solution_description, hint, max_submissions, credit, data_files, back_end, instructions, output_type, allow_any_response, what_students_see_after_success, starter_code, date_created, date_updated, enable_pair_programming, show_upload_button, verification_code, weight, min_solution_length, max_solution_length)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'''

                cursor.execute(sql, [exercise_basics["assignment"]["course"]["id"], exercise_basics["assignment"]["id"], exercise_basics["title"], exercise_basics["visible"], str(exercise_details["solution_code"]), exercise_details["solution_description"], exercise_details["hint"], exercise_details["max_submissions"], exercise_details["credit"], json.dumps(exercise_details["data_files"], default=str), exercise_details["back_end"], exercise_details["instructions"], exercise_details["output_type"], exercise_details["allow_any_response"], exercise_details["what_students_see_after_success"], exercise_details["starter_code"], format_datetime_for_db(exercise_details["date_created"]), format_datetime_for_db(exercise_details["date_updated"]), exercise_details["enable_pair_programming"], exercise_details["show_upload_button"], exercise_details["verification_code"], exercise_details["weight"], exercise_details["min_solution_length"], exercise_details["max_solution_length"]])
    ...
    def get_assignment_bulk_edit_data(self, course_id):
        ...
        for row in self.fetchall(sql, (course_id,)):
            result[row["assignment_id"]] = {
                "has_timer": bool(row["has_timer"]),
                "allow_students_view_submissions": bool(row["allow_students_view_submissions"]),
    ...
    def update_assignment_field(self, course_id, assignment_id, field_name, value):
        ...
        if field_name == "dates":
            ...
            field_map = {
                "title": "title",
                "allow_students_view_submissions": "allow_students_view_submissions",
    ...
    async def copy_course(self, existing_course_basics, new_course_title):
        ...
        sql = '''INSERT INTO courses (title, introduction, visible, passcode, email_address, highlighted, virtual_assistant_config, templates, date_created, date_updated)
                 SELECT ?, introduction, visible, passcode, email_address, highlighted, virtual_assistant_config, templates, date_created, date_updated
                 FROM courses
                 WHERE course_id = ?'''
        ...
        for assignment_basics in self.get_assignments(existing_course_basics):
            ...
            for exercise_id in old_exercise_ids:
                sql = '''INSERT INTO exercises (course_id, assignment_id, title, visible, solution_code, solution_description, hint, max_submissions, credit, data_files, back_end, instructions, output_type, what_students_see_after_success, starter_code, date_created, date_updated, enable_pair_programming, show_upload_button, verification_code, weight, min_solution_length, max_solution_length, allow_any_response)
...
#            shutil.rmtree(tmp_dir_path, ignore_errors=True)
    ...
    def get_submissions_student_for_assignment(self, course_id, assignment_id, student_id):
        sql = '''SELECT e.title AS exercise_title,
                        e.instructions AS exercise_instructions,
                        e.back_end,
                        e.solution_code,
                        s.code,
                 ...
                 WHERE s.course_id = ?
                   AND s.assignment_id = ?
                   AND s.user_id = ?
                   AND (s.passed = 1 OR e.back_end = 'multiple_choice')
                   AND e.visible = 1
                   AND a.visible = 1
                 GROUP BY e.exercise_id'''

        submissions = []
        for row in self.fetchall(sql, (course_id, assignment_id, student_id,)):
            submission = {}
            for x in ["exercise_title", "exercise_instructions", "back_end", "solution_code", "code"]:
                submission[x] = row[x]
        ...
        return sort_list_of_dicts_nicely(submissions, ["exercise_title"])
