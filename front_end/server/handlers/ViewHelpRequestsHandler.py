    ...
    def get(self, course, assignment, exercise, student_id):
        try:
            if self.is_administrator() or self.is_instructor_for_course(course) or self.is_assistant_for_course(course):
                self.render("view_request.html", courses=self.get_courses(), course_basics=self.content.get_course_basics(course), assignments=self.content.get_assignments_basics(course), assignment_basics=self.content.get_assignment_basics(course, assignment), exercises=self.content.get_exercises(course, assignment), exercise_basics=self.content.get_exercise_basics(course, assignment, exercise), exercise_details=self.content.get_exercise_details(course, assignment, exercise), help_request=self.content.get_help_request(course, assignment, exercise, student_id), exercise_help_requests=self.content.get_exercise_help_requests(course, assignment, exercise, student_id), similar_requests=self.content.compare_help_requests(course, assignment, exercise, student_id), result=None, user_info=self.get_user_info(), is_administrator=self.is_administrator(), is_instructor=self.is_instructor_for_course(course), is_assistant=self.is_assistant_for_course(course))
            else:
