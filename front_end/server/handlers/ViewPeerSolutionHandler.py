    ...
    def get(self, course, assignment, exercise):
        try:
            ...
            if not self.check_whether_should_show_exercise(course, assignment, assignment_details, assignments, courses, assignment_basics, course_basics):
                ...
                self.render("view_peer_solution.html", courses=courses, assignments=assignments, exercises=exercises, course_basics=course_basics, assignment_basics=assignment_basics, assignment_details=assignment_details, exercise_basics=exercise_basics, exercise_details=exercise_details, exercise_statuses=exercise_statuses, user_code=user_code, peer_code=peer_code, user_info=user_info, check_for_restrict_other_assignments=self.content.check_for_restrict_other_assignments(course), is_administrator=self.is_administrator(), is_instructor=self.is_instructor_for_course(course), is_assistant=self.is_assistant_for_course(course))
