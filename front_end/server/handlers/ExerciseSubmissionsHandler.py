    ...
    def get(self, course_id, assignment_id, exercise_id):
        try:
            if self.is_administrator or self.is_instructor_for_course(course_id) or self.is_assistant_for_course(course_id):
                ...
                self.render("exercise_submissions.html", courses=self.courses, course_basics=course_basics, assignments=self.get_assignments(course_basics), assignment_basics=assignment_basics, exercise_basics=self.get_exercise_basics(course_basics, assignment_basics, exercise_id), exercise_submissions=self.content.get_exercise_submissions(course_id, assignment_id, exercise_id), user_info=self.user_info, is_administrator=self.is_administrator)
            else:
