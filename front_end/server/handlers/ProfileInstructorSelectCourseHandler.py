    ...
    def get(self, user_id):
        try:
            ...
            if len(courses) > 1:
                ...
                self.render("profile_instructor.html", page="instructor", tab=None, course=courses[0][1], assignments=self.content.get_assignments_basics(courses[0][0]), instructors=self.content.get_users_from_role(courses[0][0], "instructor"), assistants=self.content.get_users_from_role(courses[0][0], "assistant"), registered_students=self.content.get_registered_students(courses[0][0]), result=None, user_info=self.get_user_info(), is_administrator=self.is_administrator(), is_instructor=self.is_instructor_for_course(courses[0][0]), is_assistant=self.is_assistant_for_course(courses[0][0]), registered_courses=self.content.get_registered_courses(user_id))
