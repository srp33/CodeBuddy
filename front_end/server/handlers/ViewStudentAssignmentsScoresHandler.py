    ...
    async def get(self, course_id, user_id):
        try:
            if self.is_administrator or await self.is_instructor_for_course(course_id) or await self.is_assistant_for_course(course_id):
                ...
                self.render("view_student_assignments_scores.html", courses=self.courses, course_basics=course_basics, assignment_statuses=await self.get_assignment_statuses(course_basics), assignment_scores=self.content.get_student_assignment_scores(course_id, user_id), user_info=self.content.get_user_info(user_id), is_administrator=self.is_administrator)
            else:
