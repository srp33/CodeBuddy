    ...
    def get(self, course, assignment, exercise, student_id):
        try:
            if self.is_administrator() or self.is_instructor_for_course(course) or self.is_assistant_for_course(course):
                ...
                self.render("student_exercise.html", student_info=student_info, student_id=student_id, courses=courses, course_basics=course_basics, assignments=assignments, assignment_basics=assignment_basics, exercises=exercises, exercise_basics=exercise_basics, assignment_details=assignment_details, exercise_details=exercise_details, tests=tests, exercise_statuses=exercise_statuses, assignment_options=assignment_options, code_completion_path=back_end["code_completion_path"], back_end_description=back_end["description"], submissions=submissions, num_submissions=num_submissions, user_info=self.get_user_info(), user_id=self.get_user_id(), next_prev_student_ids = self.content.get_next_prev_student_ids(course, student_id), is_administrator=self.is_administrator(), is_instructor=self.is_instructor_for_course(course), is_assistant=self.is_assistant_for_course(course))
            else:
