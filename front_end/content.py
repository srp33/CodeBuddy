...
#            course_scores[row["assignment_id"]] = assignment_dict
    ...
    def get_exercise_details(self, course, assignment, exercise, format_content=False):
        ...
        sql = '''SELECT code, show_code, text_output, image_output
                 FROM tests
                 WHERE course_id = ?
                   AND assignment_id = ?
                   AND exercise_id = ?'''
        ...
        for test in tests:
            exercise_dict["tests"].append({"code": test["code"], "show_code": test["show_code"], "text_output": test["text_output"], "image_output": test["image_output"]})
        ...
        if exercise_basics["exists"]:
            ...
            if exercise_details["tests"] != []:
                for i in range(len(exercise_details["tests"])):
                    sql = '''INSERT INTO tests (course_id, assignment_id, exercise_id, code, show_code, text_output)
                             VALUES (?, ?, ?, ?, ?, ?)'''

                    test_id = self.execute(sql, [exercise_basics["assignment"]["course"]["id"], exercise_basics["assignment"]["id"], exercise_basics["id"], exercise_details["tests"][i]["code"], exercise_details["tests"][i]["show_test_output"], exercise_details["tests_dict"][i]["text_output"]])
        else:
    ...
    def save_submission(self, course, assignment, exercise, user, code, text_output, image_output, passed, tests_dict, partner_id=None):
        ...
        if len(tests_dict) > 0:
            print("C", tests_dict)
            test_sql = '''INSERT INTO submission_outputs (course_id, assignment_id, exercise_id, user_id, submission_id, text_output, image_output)
                         VALUES (?, ?, ?, ?, ?, ?, ?)'''
            for test in tests_dict:
                if "image_output" not in test.keys():
                    test["image_output"] = None
                print("E", test)
