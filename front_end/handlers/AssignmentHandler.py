...
class AssignmentHandler(BaseUserHandler):
    def get(self, course, assignment):
        if self.is_administrator() or self.is_instructor_for_course(course) or self.is_assistant_for_course(course):
            ...
            try:
                ...
                start_time = self.content.get_user_assignment_start_time(course, assignment, user_info["user_id"])
                ...
                if assignment_details["allowed_ip_addresses"] and client_ip not in assignment_details["allowed_ip_addresses"]:
                    ...
                    self.render("unavailable_assignment.html", courses=self.content.get_courses(), assignments=self.content.get_assignments(course), course_basics=self.content.get_course_basics(course), assignment_basics=self.content.get_assignment_basics(course, assignment), error="start", start_date=assignment_details["start_date"].strftime("%c"), user_info=user_info)
                    ...
                    self.render("unavailable_assignment.html", courses=self.content.get_courses(), assignments=self.content.get_assignments(course), course_basics=self.content.get_course_basics(course), assignment_basics=self.content.get_assignment_basics(course, assignment), error="due", due_date=assignment_details["due_date"].strftime("%c"), user_info=user_info)
                    ...
                    self.render("assignment.html", courses=self.content.get_courses(False), assignments=self.content.get_assignments(course, False), exercises=self.content.get_exercises(course, assignment, False), exercise_statuses=self.content.get_exercise_statuses(course, assignment, user_info["user_id"]), course_basics=self.content.get_course_basics(course), assignment_basics=self.content.get_assignment_basics(course, assignment), assignment_details=assignment_details, curr_datetime=curr_datetime, start_time=start_time, user_info=user_info)
    ...
    def post(self, course, assignment):
        try:
            ...
            self.content.set_user_assignment_start_time(course, assignment, user_info["user_id"], start_time)

            self.render("assignment.html", courses=self.content.get_courses(show), assignments=self.content.get_assignments(course, show), exercises=self.content.get_exercises(course, assignment, show), exercise_statuses=self.content.get_exercise_statuses(course, assignment, user_info["user_id"]), course_basics=self.content.get_course_basics(course), assignment_basics=self.content.get_assignment_basics(course, assignment), assignment_details=self.content.get_assignment_details(course, assignment, True), curr_datetime=dt.datetime.now(), start_time=self.content.get_user_assignment_start_time(course, assignment, user_info["user_id"]), user_info=user_info)
        except Exception as inst:
            render_error(self, traceback.format_exc())
