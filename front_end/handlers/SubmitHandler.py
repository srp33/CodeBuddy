...
class SubmitHandler(BaseUserHandler):
    async def post(self, course, assignment, exercise):
        out_dict = {"txt_output": "", "jpg_output": "", "diff": "", "passed": False, "submission_id": ""}
        ...
        try:
            ...
            txt_output, jpg_output, tests = exec_code(self.settings_dict, code, exercise_details, self.request)
            ...
            diff, passed, test_outcomes = check_exercise_output(exercise_details, txt_output, jpg_output, tests)

            out_dict["txt_output"] = format_output_as_html(txt_output.strip())
            out_dict["jpg_output"] = jpg_output
            out_dict["tests"] = test_outcomes
            out_dict["diff"] = format_output_as_html(diff)
            out_dict["passed"] = passed
            out_dict["submission_id"] = self.content.save_submission(course, assignment, exercise, user_id, code, txt_output, jpg_output, passed, tests, partner_id)
