    ...
    def get(self, course):
        ...
        if self.is_administrator() or self.is_instructor_for_course(course) or self.is_assistant_for_course(course):
            try:
                ...
                self.render("course_admin.html", courses=self.content.get_courses(True), assignments=assignments, course_basics=self.content.get_course_basics(course), course_details=self.content.get_course_details(course, True), course_scores=self.content.get_course_scores(course, assignments), user_info=self.get_user_info(), is_administrator=self.is_administrator(), is_instructor=self.is_instructor_for_course(course))
            ...
            try:
                self.render("course.html", courses=self.content.get_courses(False), assignments=self.content.get_assignments(course, False), assignment_statuses=self.content.get_assignment_statuses(course, self.get_user_id()), course_basics=self.content.get_course_basics(course), course_details=self.content.get_course_details(course, True), curr_datetime=dt.datetime.now(), user_info=self.get_user_info())
            except Exception as inst:
                render_error(self, traceback.format_exc())
